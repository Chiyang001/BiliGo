...
AI 回复位于 **统一 AI 回复设置** 页面，支持按平台独立启用：B 站私信、B 站评论、小红书、微博、抖音和闲鱼。

1. 先完成目标平台登录，确认平台页面显示已登录。
2. 在 AI 提供商中填写协议格式、API Base URL、模型名称和 API Key，点击 **测试连接** 验证后保存。
3. 如需让 AI 参考业务资料，在知识库配置中创建知识库，可直接输入文本或上传 Markdown，再分配给目标平台。
4. 在平台设置中打开对应平台的 **开放此平台** 开关并保存。
5. 返回对应平台页面，点击 **开始监控**。AI 设置保存不等于监控已启动。

启用某个平台的 AI 后，该平台原有的默认回复和关键词规则会锁定并停用；未启用 AI 的平台不受影响。AI 生成失败不会自动回退到传统默认回复，请检查 API 地址、模型、密钥、额度和网络，并查看运行日志。

如果启动前自动保存提示“已启用 AI 模式，无法修改默认回复设置”，这是传统表单被锁定的提示，不代表 AI 配置损坏；继续点击开始监控即可，仍需以平台登录状态和日志中的启动结果为准。

### 小红书、微博与闲鱼消息

两个平台都使用独立的浏览器登录会话，操作步骤如下：

1. 从平台切换菜单进入“小红书私信”“微博私信”或“闲鱼消息”。
2. 点击 **打开登录窗口**，在浏览器中完成登录，回到页面确认账号状态。
3. 设置检测间隔、发送间隔、仅回复新消息和单用户回复次数。
4. 未启用 AI 时，再配置默认回复和关键词规则；启用 AI 则在统一 AI 页面完成配置。
5. 点击 **开始监控**，确认状态和日志正常后发送测试私信。

各平台的 Cookie、规则、统计和日志互相独立。首次启动会建立历史消息基线，启用“仅回复新消息”时不会批量处理旧消息。微博、小红书和闲鱼当前使用文字回复，图片回复不可用。各平台均可通过“导出配置”下载 JSON 配置包，并通过“导入配置”选择替换或追加规则；配置包不包含登录会话。

### 五平台相互导入

B 站私信、B 站评论、抖音、小红书、微博和闲鱼页面均可从其他任一平台导入。支持替换现有规则或按关键词追加新规则，并同步通用监控参数和文字默认回复。登录凭据、Cookie、会话、统计和日志不会跨平台复制；图片规则会转换为可移植的文字规则。目标平台启用 AI 时，请先关闭该平台 AI 开关再导入传统规则。
...
> 在 **20260830** 稳定版基础上演进的新版本，功能持续开发中，尚未正式发布。

**进行中**:
- 🚀 版本号升级为 **V3 Ultra**，整合 B 站私信、评论与抖音私信能力
- 📅 正式发布日期待定
