...
class CommentReplySystem:
    def __init__(self):
        self.config = {
            ...
            'comment_check_interval': 30,  # 评论检查间隔（秒）
            'comment_send_delay': 2.0,  # 评论回复发送间隔（秒）
    ...
    def get_my_videos(self, page=1, page_size=10):
        ...
        try:
            ...
            response = self.bili_api.session.get(url, params=params, timeout=5)
            response.raise_for_status()
            return response.json()
        except Exception as e:
            self.add_log(f"获取视频列表失败: {e}", 'error')
            return None
    ...
    def get_video_comments(self, oid, page=1, page_size=20):
        ...
        try:
            ...
            self.add_log(f"获取视频评论失败: {e}", 'error')
            return None
    ...
    def process_comments(self):
        ...
        try:
            # 获取我的视频列表
            videos_data = self.get_my_videos(page=1, page_size=5)  # 只检查最新5个视频
            if not videos_data or videos_data.get('code') != 0:
                self.add_log("获取视频列表失败", 'error')
                return
            ...
            for video in videos:
                ...
                # 获取视频评论
                comments_data = self.get_video_comments(video_id, page=1, page_size=10)
                ...
                for reply in replies:
                    ...
                    try:
                        ...
                        commenter_name = reply.get('member', {}).get('uname', '未知用户')
                        ...
                        # 标记为已处理
                        self.comment_cache[cache_key] = comment_time
                        
                        self.add_log(f"检测到新评论: {commenter_name} 在 {video_title} 中评论: {comment_text[:50]}...", 'info')
                        ...
                        if matched_rule:
                            ...
                            self.add_log(f"匹配到规则: {matched_rule.get('name', '未命名规则')}", 'info')
                        ...
                        if reply_message:
                            ...
                            if success:
                                self.add_log(f"成功回复 {commenter_name} 的评论", 'success')
                            else:
                                self.add_log(f"回复 {commenter_name} 的评论失败", 'error')
    ...
    def monitor_comments(self):
        ...
        while self.monitoring:
            try:
                ...
                # 等待下次检查
                check_interval = self.config.get('comment_check_interval', 30)
                ...
                self.add_log(f"评论监控异常: {e}", 'error')
                time.sleep(10)  # 出错后等待10秒再继续
        
        self.add_log("评论自动回复监控已停止", 'warning')
